...
public class BanditOnInitialEntitySpawnProcedure {
	public static void execute(Entity entity) {
		...
		if (Math.random() < (1) / ((float) 3)) {
			ran_weapon = Mth.nextInt(RandomSource.create(), 1, 4);
			if (ran_weapon == 1) {
			...
			} else if (ran_weapon == 3) {
				if (entity instanceof LivingEntity _entity) {
					ItemStack _setstack = new ItemStack(InvincibleCraftModItems.GAUNTLET.get()).copy();
					_setstack.setCount(1);
					_entity.setItemInHand(InteractionHand.MAIN_HAND, _setstack);
					if (_entity instanceof Player _player)
						_player.getInventory().setChanged();
		...
		if (random <= 3) {
			...
			if (entity instanceof LivingEntity _livingEntity18 && _livingEntity18.getAttributes().hasAttribute(Attributes.ATTACK_DAMAGE))
				_livingEntity18.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(1);
			if (entity instanceof LivingEntity _livingEntity19 && _livingEntity19.getAttributes().hasAttribute(Attributes.MOVEMENT_SPEED))
				_livingEntity19.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(0.15);
