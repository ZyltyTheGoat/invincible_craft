...
public class ThraggUpslamProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity target) {
		...
		} else if ((entity instanceof ThraggExiledEntity _datEntI ? _datEntI.getEntityData().get(ThraggExiledEntity.DATA_AttackDuration) : 0) == 3) {
		...
		} else if ((entity instanceof ThraggExiledEntity _datEntI ? _datEntI.getEntityData().get(ThraggExiledEntity.DATA_AttackDuration) : 0) >= 5) {
			{
				Entity _ent = entity;
				_ent.teleportTo((target.getX()), (target.getY() + 1.4), (target.getZ()));
				if (_ent instanceof ServerPlayer _serverPlayer)
