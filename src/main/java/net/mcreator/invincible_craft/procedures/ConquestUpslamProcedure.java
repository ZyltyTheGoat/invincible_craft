...
public class ConquestUpslamProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity target) {
		...
		if ((entity instanceof ConquestEntity _datEntI ? _datEntI.getEntityData().get(ConquestEntity.DATA_AttackDuration) : 0) == 1) {
			...
			target.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("invincible_craft:viltrumite_punch"))), entity), 20);
			...
			if (entity instanceof ConquestEntity _datEntSetI)
				_datEntSetI.getEntityData().set(ConquestEntity.DATA_UpslamCooldown, 120);
		}
	}
}
