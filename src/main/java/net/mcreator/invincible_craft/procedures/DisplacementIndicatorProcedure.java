...
public class DisplacementIndicatorProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (DisplacementIndicatorProcedure.bufferBuilder == null || !DisplacementIndicatorProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (DisplacementIndicatorProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					DisplacementIndicatorProcedure.mode = mode;
					DisplacementIndicatorProcedure.format = format;
					DisplacementIndicatorProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					DisplacementIndicatorProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					DisplacementIndicatorProcedure.mode = mode;
					DisplacementIndicatorProcedure.format = format;
					DisplacementIndicatorProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					DisplacementIndicatorProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		DisplacementIndicatorProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			DisplacementIndicatorProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
				...
				if (world instanceof ClientLevel) {
					for (Entity entityiterator : ((ClientLevel) world).entitiesForRendering()) {
						if (((entity.getCapability(InvincibleCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new InvincibleCraftModVariables.PlayerVariables())).displacement_target).equals(entityiterator.getStringUUID())) {
							...
							renderShape(ReturnIndicatorShapeProcedure.execute(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() + 0.5), (entityiterator.getZ()),
