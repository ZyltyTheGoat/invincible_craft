...
public class KillCannonOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(InvincibleCraftModMobEffects.DENY.get())) {
			if (entity instanceof KillCannonEntity _datEntSetL)
				_datEntSetL.getEntityData().set(KillCannonEntity.DATA_denied, true);
		} else {
			if (entity instanceof KillCannonEntity _datEntSetL)
				_datEntSetL.getEntityData().set(KillCannonEntity.DATA_denied, false);
		}
		if (!((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			if (!(entity instanceof KillCannonEntity _datEntL6 && _datEntL6.getEntityData().get(KillCannonEntity.DATA_denied))) {
