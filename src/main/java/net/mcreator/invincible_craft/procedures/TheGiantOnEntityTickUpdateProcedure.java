...
public class TheGiantOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		double rand = 0;
		double distance = 0;
		if (entity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(InvincibleCraftModMobEffects.DENY.get())) {
			if (entity instanceof TheGiantEntity _datEntSetL)
				_datEntSetL.getEntityData().set(TheGiantEntity.DATA_denied, true);
		} else {
			if (entity instanceof TheGiantEntity _datEntSetL)
				_datEntSetL.getEntityData().set(TheGiantEntity.DATA_denied, false);
		}
		if (!((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (!(entity instanceof TheGiantEntity _datEntL12 && _datEntL12.getEntityData().get(TheGiantEntity.DATA_denied))) {
				if (distance <= 14) {
				...
				if ((entity instanceof TheGiantEntity _datEntI ? _datEntI.getEntityData().get(TheGiantEntity.DATA_IA) : 0) == 21) {
					{
						...
						for (Entity entityiterator : _entfound) {
							if (!(entityiterator == entity) && entityiterator instanceof LivingEntity) {
								entityiterator.hurt(
										new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("invincible_craft:impact"))), entity),
										(float) (entity instanceof LivingEntity _livingEntity35 && _livingEntity35.getAttributes().hasAttribute(Attributes.ATTACK_DAMAGE) ? _livingEntity35.getAttribute(Attributes.ATTACK_DAMAGE).getBaseValue() : 0));
							}
						}
					}
				}
