...
public class AtomicBubbleRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (AtomicBubbleRenderProcedure.bufferBuilder == null || !AtomicBubbleRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (AtomicBubbleRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					AtomicBubbleRenderProcedure.mode = mode;
					AtomicBubbleRenderProcedure.format = format;
					AtomicBubbleRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					AtomicBubbleRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					AtomicBubbleRenderProcedure.mode = mode;
					AtomicBubbleRenderProcedure.format = format;
					AtomicBubbleRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					AtomicBubbleRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		AtomicBubbleRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			AtomicBubbleRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if ((entity.getCapability(InvincibleCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new InvincibleCraftModVariables.PlayerVariables())).atom_eve_atomic_bubble_timer != 0) {
			...
			if (target(2)) {
				renderShape(shape(), (entity.getX()), (entity.getY() + entity.getBbHeight() / 2), (entity.getZ()), 0, 0, 0,
