			...
			if (entity instanceof ServerPlayer _ent) {
				...
				NetworkHooks.openScreen((ServerPlayer) _ent, new MenuProvider() {
					...
					public Component getDisplayName() {
						return Component.literal("ArtRosenbaumGUIPage2");
					}

					@Override
					public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
						return new ArtRosenbaumGUIPage2Menu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
					}
				}, _bpos);
			}
		} else if ((entity.getCapability(InvincibleCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new InvincibleCraftModVariables.PlayerVariables())).art_page == 2) {
			{
				double _setval = 1;
				entity.getCapability(InvincibleCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
					capability.art_page = _setval;
