	...
	public static void execute(Entity entity) {
		...
		if (new Object() {
			...
			if (((entity.getCapability(InvincibleCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new InvincibleCraftModVariables.PlayerVariables())).power).equals("Viltrumite")
				...
				if (!entity.onGround()) {
					if (!(entity instanceof LivingEntity _livEnt3 && _livEnt3.hasEffect(InvincibleCraftModMobEffects.STUN.get()))) {
						{
						...
						entity.getPersistentData().putBoolean("Ascend", true);
						entity.getPersistentData().putBoolean("WasFlying", true);
					}
				}
			}
		}
