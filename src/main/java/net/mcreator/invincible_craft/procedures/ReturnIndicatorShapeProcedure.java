...
public class ReturnIndicatorShapeProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (ReturnIndicatorShapeProcedure.bufferBuilder == null || !ReturnIndicatorShapeProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (ReturnIndicatorShapeProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					ReturnIndicatorShapeProcedure.mode = mode;
					ReturnIndicatorShapeProcedure.format = format;
					ReturnIndicatorShapeProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					ReturnIndicatorShapeProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					ReturnIndicatorShapeProcedure.mode = mode;
					ReturnIndicatorShapeProcedure.format = format;
					ReturnIndicatorShapeProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					ReturnIndicatorShapeProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
